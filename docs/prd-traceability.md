...
| 3a | Agent runs anywhere, connects out only (no inbound) | `prd-compliance.test.ts` (portless agent, agent env has DATABASE_URL/SERVER_URL) |
...
| C2 | Auth & authorization boundary | `prd-session-lifecycle.test.ts` (401 anon, 403 cross-user, server-injected user_id), `cors.test.ts`, `auth.test.ts` |
...
| C8 | LLM key via `.env`, documented | `prd-compliance.test.ts` (C8) |
| C10 | Compose has server+UI, db, Electric, agent containers | `prd-compliance.test.ts` (C5/C10) |
| C11 | Server+UI publish ports; agent container opens none | `prd-compliance.test.ts` (C11 + source-level portless) |
| C12 | `docker compose up` builds with only a `.env`; project builds | `prd-compliance.test.ts` (migrate one-shot), `prd-build.test.ts` |
...
| D2 | README links a web-viewable demo (Loom) | `prd-compliance.test.ts` (D2 — `it.todo`, pending recording) |
| D3 | AI coding-agent config dir included | `prd-compliance.test.ts` (D3: `CLAUDE.md`) |

## Notes

- **D2 (Loom)** is the one open item — tracked as a `todo` test, not a passing
  assertion, because the video is not yet recorded.
- **Full `docker compose up` boot** is validated structurally by parsing
  `docker compose config` (no daemon needed); an actual end-to-end boot is a
  manual smoke (performed during development).
- The integration suites reset the `public` schema on the shared test database
  (`:54321`). Point `TEST_DATABASE_URL` at a throwaway database to avoid wiping
  a running stack's data.
